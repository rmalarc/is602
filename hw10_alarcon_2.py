...
class LinearRegression:
    ...
    def equation(self):
        return "y = %.2fx %+0.2f" % (self.beta, self.alpha)
    ...
    def print_equation(self):
        ...
        """
        print("Regression equation: " +self.equation())
    ...
    def show_plot(self):
        ...
        for x in self.data['x']:
            f_x.append(x*self.beta+self.alpha)

        plt.plot(self.data['x'],self.data['y'],'r--',self.data['x'],f_x)

        plt.xlabel('Body Weight')
        plt.ylabel('Brain Weight')
        plt.title('Brain Weight by Body Weight Regression')
        plt.text(1200,2500,self.equation())
        plt.grid(True)
        plt.show()
